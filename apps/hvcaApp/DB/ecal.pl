...
# ECAL_TOP_01 01 1 01 01 00 03 00  1548.0   1.5   1.3    61.0    61.0  2550.0  2500.0
# ECAL_BOT_01 02 1 01 01 00 03 00  1548.0   1.5   1.3    61.0    61.0  2550.0  2500.0

$crate="14";
$mainframe="01";


@groupNames = split(':',"TOP:BOT");

for $ch (0..51) {
    $channel = ($ch % 26) + 1;
    $gr = int($ch / 26) + 1;
    $br = int($ch / 12);
    $grName = $groupNames[$gr-1];
         # ECAL_BOT_01                02           1 01 01 00 03 00  1548.0   1.5   1.3    61.0    61.0  2550.0  2500.0
    printf "ECAL_${grName}_%02d 0${gr} 1 01 01 %02d 03 %02d  1548.0   1.5   1.3    61.0    61.0  2550.0  2500.0\n", $channel, $br, $ch%12;
}
...
print <<EOD
record(bo, "B_HVON_SET_${CRATE}_${MAINFRAME}") {
   field(DTYP,"CAEN_HV")
   field(ZNAM,"HV/OFF")
...
record(bi, "B_HVVALID_${CRATE}_${MAINFRAME}") {
   field(DTYP,"CAEN_HV")
...
record(bi, "B_HVALARM_${CRATE}_${MAINFRAME}") {
   field(DTYP,"CAEN_HV")
   field(ZNAM,"enabled")
   field(DESC,"binary input record")
   field(ONAM,"disabled")
...
record(bi, "B_HVSTATUS_${CRATE}_${MAINFRAME}") {
   field(DTYP,"CAEN_HV")
   field(ZNAM,"HV/OFF")
   field(DESC,"binary input record")
   field(ONAM,"HV/ON")
...
record(calc, "B_HVOC_${CRATE}_${MAINFRAME}") {
   field(LOLO,"-0.1")
   field(CALC,"ABS(A-B)")
   field(LLSV,"MAJOR")
   field(HIHI,"0.1")
   field(SCAN,"1 second")
   field(HHSV,"MAJOR")
   field(INPA,"B_HVSTATUS_${CRATE}_${MAINFRAME} NPP NMS")
   field(LSV,"MAJOR")
   field(LOW,"-0.1")
   field(INPB,"B_HVON_SET_${CRATE}_${MAINFRAME} NPP NMS")
   field(HIGH,"0.1")
   field(HSV,"MAJOR")
}
EOD
