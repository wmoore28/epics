...
void blockCloner::blockCloner(Char_t *TemplateFileName,Char_t *identifier=NULL,Char_t *start="<widget",Char_t *stop="</widget"){
  ...
  for(Int_t n=fCloneStart;n<fNlines;n++){ 
    if(strstr(fAllLines[n]->Data(),start)){ 
      nBlockOpen++;
      //cout << n << " nBlockOpen: " << nBlockOpen << " " << fAllLines[n]->Data();
    }
    if(strstr(fAllLines[n]->Data(),stop)){
      nBlockClose++;
      //cout << n << " nBlockClose: " << nBlockClose << " " << fAllLines[n]->Data();
    }
...
Char_t *blockCloner::GetBlockLine(Int_t line, Char_t *find=NULL, Char_t*replace=""){
...
Int_t blockCloner::GetCloneSize(){return (fCloneEnd-fCloneStart)+1;}
Int_t blockCloner::GetFileEnd(){return fNlines;}
