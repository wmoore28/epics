...
sub init_gui
  { 
    ...
    #
    #    Radio Button for Beam Type
    #
    my %RadioBut;
    my(@pl)=qw/-side top -pady 18 -anchor w/;
    my $BeamTypeTmp = "Electron Beam" ;
    $RadioBut{$BeamTypeTmp} = $LeftFrame->Radiobutton(
					  -text     => $BeamTypeTmp,
					  -variable => \$BeamType,
					  -value    => $BeamTypeTmp,
					  -command  => sub { print "Expecting $BeamType \n" ; },
					  -relief   => 'flat')->pack(@pl);
    my(@pl)=qw/-side top -pady 18 -anchor w/;
    my $BeamTypeTmp = "Photon Beam" ;
    $RadioBut{$BeamType} = $RightFrame->Radiobutton(
					  -text     => $BeamTypeTmp,
					  -variable => \$BeamType,
					  -value    => $BeamTypeTmp,
					  -command  => sub { print "Expecting $BeamType \n" ; },
...
sub do_read_back
  {
    ...
    my $BeamTypeFlag;
    ( $Err, $BeamTypeFlag ) = Pezca::GetDouble( $ValueRec{"BEAM_TYPE"} ) ;
    if ( $BeamTypeFlag == 0 ) {
      $BeamType = "Electron Beam";
    } else {
      $BeamType = "Photon Beam";
    }
...
sub set_rec
  {
    ...
    my $BeamTypeFlag = -1;
    if ( $$BeamType eq "Electron Beam" ) {
      $BeamTypeFlag = 0;
    } elsif ( $$BeamType eq "Photon Beam" ) {
      $BeamTypeFlag = 1;
    } else {
      print "Wrong type of BEAM [$$BeamType] \n";
    }
    if ( $$MinCurrFlag )
      {
...
#	my $CurRecName = $ValueRec{'BPM'} ;
#	if ( $BeamTypeFlag == 0 ) {
#	  $CurRecName = $ValueRec{'FCUP'} ;
#	}
	
#	$CalcRec{"Reg"}{"INPA"}  = $CurRecName ;
	$CalcRec{"Reg"}{"INPA"}  = $ValueRec{'CURRENT'} ;
	$CalcRec{"Reg"}{"INPG"}  = "$MinCurrNA" ;
	$CalcRec{"Reg"}{"G"}     = "$MinCurrNA" ;
	$CalcRec{"Reg"}{"CALC"} .= "\&\&A\>G" ;
	
	$CalcRec{"Mol"}{"INPA"}  = $ValueRec{'BPM'} ;
...
#	    $CalcRec{"Reg"}{"INPF"}  = $CurRecName ;
	    $CalcRec{"Reg"}{"INPF"}  = $ValueRec{'CURRENT'} ;
	    $CalcRec{"Reg"}{"INPL"}  = "$MaxCurrNA" ;
	    $CalcRec{"Reg"}{"L"}     = "$MaxCurrNA" ;
	    $CalcRec{"Reg"}{"CALC"} .= "\&\&F\<L" ;	
	
	    $CalcRec{"Mol"}{"INPD"}  = $ValueRec{'BPM'} ;
    ...
    $Err |= Pezca::PutDouble( $ValueRec{"BEAM_TYPE"}, $BeamTypeFlag );
