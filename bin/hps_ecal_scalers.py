...
for crate in CRATES:
    ...
    for slot in SLOTS:
        ...
        for chan in CHANS:
            if CHARGE:
              ss[chan]=pv.PV('B_HW_HPS%d_FADC_Sl%.2d_Ch%.2d:q'%(icrate,slot,chan))
            else:
              ss[chan]=pv.PV('B_HW_HPS%d_FADC_Sl%.2d_Ch%.2d:c'%(icrate,slot,chan))
...
def calcRates(rates):
    ...
    for ii in range(len(XVALS)):
        ...
        if abs(yy)<3 and (xx>=-11 and xx<=1):
...
#    atexit.register(__atexit__)
    ...
    hhr=TH2D('hhr',';X;Y',46,-22,24,11,-5,6)
    hi=TH2I('hi',';X;Y',46,-22,24,11,-5,6)
    setupHists([hh,hi,hhr])
...
#        try:
            ...
            for ii in range(len(zvals)):
                if ACCUMULATE:
                  hh.SetBinContent(xax.FindBin(XVALS[ii]),yax.FindBin(YVALS[ii]),zvals[ii]+hh.GetBinContent(xax.FindBin(XVALS[ii]),yax.FindBin(YVALS[ii])))
                  hi.SetBinContent(xax.FindBin(XVALS[ii]),yax.FindBin(YVALS[ii]),zvals[ii]+hi.GetBinContent(xax.FindBin(XVALS[ii]),yax.FindBin(YVALS[ii])))
                else:
                  hh.SetBinContent(xax.FindBin(XVALS[ii]),yax.FindBin(YVALS[ii]),zvals[ii])
                  hhr.SetBinContent(xax.FindBin(XVALS[ii]),yax.FindBin(-YVALS[ii]),zvals[ii])
