...
///printf("ioc:Dsc2ReadScalers\n");
  Dsc2ReadScalers(ptr_c, it, nchannels);
///printf("ioc:Dsc2ReadScalers2\n");
 }
...
// for(unsigned int i=0;i<ptr_c->crateBoards.size();i++){
 for( map<int, JlabBoard *>::iterator it=ptr_c->crateBoards.begin() ; it!= ptr_c->crateBoards.end();  ++it){
  if(!(it->second))continue;
...
#ifdef JSCALER_DEBUG
 printf("111  slot=%d  msgclient_addr=%p buf_addr=%p\n",it->first, ptr_c->crateMsgClient, &buf);
#endif
 ///  printf("len=%d\n",len);
