...
//--------------------------------------------------------------------
bool devSnmp_oid::hasReadingDouble(void)
{
  return(reading.has_double);
}
//--------------------------------------------------------------------
bool devSnmp_oid::hasReadingLong(void)
{
  return(reading.has_long);
}
//--------------------------------------------------------------------
void devSnmp_oid::clearData(void)
{
  epicsMutexLock(valMutex);

  if (reading.read_string) reading.read_string[0] = 0;
  reading.valid = false;
...
//--------------------------------------------------------------------
void devSnmp_oid::storeData(netsnmp_variable_list *var)
{
  epicsMutexLock(valMutex);
  ...
  switch (var->type) {
      ...
      if (var->val.floatVal) {
        reading.read_double = (*var->val.floatVal);
        reading.has_double = true;
      }
      break;
    case ASN_OPAQUE_DOUBLE:
      if (var->val.doubleVal) {
        reading.read_double = (*var->val.doubleVal);
        reading.has_double = true;
      }
      break;
#endif
    ...
    case ASN_UNSIGNED:
      if (var->val.integer) {
        reading.read_long = (*var->val.integer);
        reading.has_long = true;
      }
      break;
  }
...
//--------------------------------------------------------------------
bool devSnmp_oid::getValueDouble(double *value)
{
  ...
  epicsMutexLock(valMutex);
  (*value) = reading.read_double;
  epicsMutexUnlock(valMutex);
  return(true);
}
//--------------------------------------------------------------------
bool devSnmp_oid::getValueLong(long *value)
{
...
void devSnmp_oid::setDataLength(int length)
{
  ...
  if (! reading.read_string) {
    reading.read_string = new char[data_len];
    reading.read_string[0] = 0;
  } else {
    char *newval = new char[data_len];
    copy_string(newval,data_len,reading.read_string);
    delete [] reading.read_string;
    reading.read_string = newval;
  }
...
void devSnmp_oid::report(int level, char *match)
{
  ...
  double D;
  if (getValueDouble(&D))
    printf("    Double reading   : %.10g\n",D);
  else
    printf("    Double reading   : (n/a)\n");

  long L;
  if (getValueLong(&L))
    printf("    Long reading     : %ld\n",L);
  else
    printf("    Long reading     : (n/a)\n");
...
{
  // error if we have no oid object
  if (! pOurOID) {
    strcpy(lastError,"oid object is NULL");
    return(false);
  }
...
static long snmpAiInit(struct aiRecord *pai)
{
  ...
  // disallow SCAN I/O Intr
  if (pai->scan == menuScanI_O_Intr) {
    recGblRecordError(S_db_badField,(void *)pai,
                     "devSnmpAi (init_record) Illegal SCAN field value, do not use I/O Intr");
    return(S_db_badField);
  }
...
static long snmpLiInit(struct longinRecord *pli)
{
  ...
  // disallow SCAN I/O Intr
  if (pli->scan == menuScanI_O_Intr) {
    recGblRecordError(S_db_badField,(void *)pli,
                     "devSnmpLi (init_record) Illegal SCAN field value, do not use I/O Intr");
    return(S_db_badField);
  }
...
static long snmpSiInit(struct stringinRecord *psi)
{
  ...
  // disallow SCAN I/O Intr
  if (psi->scan == menuScanI_O_Intr) {
    recGblRecordError(S_db_badField,(void *)psi,
                     "devSnmpSi (init_record) Illegal SCAN field value, do not use I/O Intr");
    return(S_db_badField);
  }
...
static epicsStatus snmpWfInit(struct waveformRecord *pwf)
{
  ...
  // disallow SCAN I/O Intr
  if (pwf->scan == menuScanI_O_Intr) {
    recGblRecordError(S_db_badField,(void *)pwf,
                     "devSnmpWf (init_record) Illegal SCAN field value, do not use I/O Intr");
    return(S_db_badField);
  }
